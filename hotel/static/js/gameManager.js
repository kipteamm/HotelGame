...
async function startGame() {
    ...
    const response = await fetch("/api/game/start", {method: "PATCH", headers: {"Authorization": `Bearer ${getCookie("se_to")}`}});
...
async function leaveGame() {
    const response = await fetch("/api/game/leave", {method: "DELETE", headers: {"Authorization": `Bearer ${getCookie("se_to")}`}});
